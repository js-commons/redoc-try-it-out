...
describe('StyleMatcherConfig', () => {
    test('test that config can be overridden', () => {
        ...
        }, swaggerConfig, redocTryItOutConfig);
        
        expect(config.summaryTargetSelector).toEqual("#swagger-ui other-summaryTargetSelector")
        expect(config.authBtnSourceSelector).toEqual("#redoc-container other-authBtnSourceSelector");
        expect(config.apiContentSourceSelector).toEqual("#redoc-container other-apiContentSourceSelector");
    });
});
