export const Config = {
    ...
    parseOptions: <THIZ extends Record<string, any>, T extends Record<string, any>>(thiz: THIZ, options: T) => {
        Object.keys(options).forEach(key => {
            // We add underscore to the key if its not a readable property in thiz
            // to enable setting private variables, that begin with underscore.
            // This was previously using the `in` keyword which returned true if field was inherited.
